...
我们仅仅在if语句的上下文环境中使用变量`bar`,所以我们将它声明在if块儿的内部是有些道理的。然而，当使用`var`时，我们在何处声明变量是无关紧要的，因为它们总是属于外围作用域。这个代码段实质上为了代码风格的原因"假冒"了块儿作用域，并依赖于我们要管好自己，不在要在这个作用域的其他地方以外地使用`bar`。
